...
//        logger.info("Computing links ...");
        ...
        for (String sourceInstanceUri : source.getAllUris()) {
            counter++;
            if (counter % 1000 == 0) {
//                logger.info("Processed " + (counter * 100 / size) + "% of the links");
                //get key
            ...
            for (int ids = 0; ids < blockIds.size(); ids++) {
                ...
                //logger.info(sourceInstanceUri+" is to compare with blocks "+blocksToCompare);
...
//        logger.info("Cmin = "+necessaryComparisons+"; C = "+comparisons);
    ...
    public static void main(String args[]) {
        ...
        //System.out.println("Mapping = "+bm.getMapping(source, target, "?x", "?y", "euclidean(x.lat|lon, y.lat|lon)", 0.5));
    }
