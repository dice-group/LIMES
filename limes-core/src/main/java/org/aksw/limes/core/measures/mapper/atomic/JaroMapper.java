...
public class JaroMapper extends AtomicMapper {
    ...
    private Mapping runLenghtOnly(Map<String, Set<String>> sourceMap, Map<String, Set<String>> targetMap, double threshold) {
        Jaro j = new Jaro();
        ...
        for (Integer sourceLength : sourceLengthIndex.keySet()) {
            for (Integer targetLength : targetLengthIndex.keySet()) {
                ...
                maxSourceLength = getMaxComparisonLength((double) sourceLength, threshold, Math.min(sourceLength, targetLength));
                maxTargetLength = getMaxComparisonLength((double) targetLength, threshold, Math.min(sourceLength, targetLength));
                //length-aware filter
    ...
    private Mapping run(Map<String, Set<String>> sourceMap, Map<String, Set<String>> targetMap, double threshold) {
        Jaro j = new Jaro();
        Set<String> source = sourceMap.keySet();
        Set<String> target = targetMap.keySet();
        Map<Integer, Set<String>> sourceLengthIndex = getLengthIndex(source);
...
//        Map<String, Set<Character>> targetCharacterMap = getChars(target);
        ...
        for (Integer sourceLength : sourceLengthIndex.keySet()) {
            for (Integer targetLength : targetLengthIndex.keySet()) {
                maxTargetLength = getMaxComparisonLength((double) sourceLength, threshold, Math.min(sourceLength, targetLength));
                maxSourceLength = getMaxComparisonLength((double) targetLength, threshold, Math.min(sourceLength, targetLength));
...
//                            if everything maps
                                    ...
                                    transpositions = Jaro.getTranspositions(sourceMappingCharacters, targetMappingCharacters);
                                    if (transpositions >= 0) {
                                        ...
                                        similarity = ((sourceMappingCharacters.size() / (float) sourceLength)
                                                ...
                                                + ((sourceMappingCharacters.size() - transpositions) / (float) sourceMappingCharacters.size())) / 3.0;
                                        if (similarity >= threshold) {
                                            for (String sourceUri : sourceMap.get(s)) {
                                                for (String targetUri : targetMap.get(t)) {
                                                    result.add(sourceUri, targetUri, similarity);
        ...
        System.out.println(characterFilterCount + " = " + ((double) characterFilterCount) / (source.size() * target.size()));
        return result;
    }

    private Mapping runWithoutPrefixFilter(Map<String, Set<String>> sourceMap, Map<String, Set<String>> targetMap, double threshold) {
        Jaro j = new Jaro();
        Set<String> source = sourceMap.keySet();
        Set<String> target = targetMap.keySet();
        Map<Integer, Set<String>> sourceLengthIndex = getLengthIndex(source);
...
//        Map<String, Set<Character>> targetCharacterMap = getChars(target);
        ...
        System.out.println(lengthFilterCount
                + " = " + ((double) lengthFilterCount) / (source.size()
                * target.size()));
//        System.out.println(prefixFilterCount
//                + " = " + ((double) prefixFilterCount) / (source.size()
//                * target.size()));
        System.out.println(characterFilterCount
                + " = " + ((double) characterFilterCount) / (source.size()
                * target.size()));
        return result;
    }
    ...
    private void deduplicationTest(int sourceSize, double threshold) {
        Map<String, Set<String>> sourceMap = generateRandomMap(sourceSize);
...
//        System.out.println("Mapping size : " + (m1.getNumberofMappings()));
        ...
        begin = System.currentTimeMillis();
        Mapping m2 = runLenghtOnly(sourceMap, sourceMap, threshold);
        end = System.currentTimeMillis();
...
//        System.out.println("Mapping size : " + (SetOperations.difference(m1, m2)));
        ...
        begin = System.currentTimeMillis();
        m2 = runWithoutPrefixFilter(sourceMap, sourceMap, threshold);
        end = System.currentTimeMillis();
...
//        System.out.println("Mapping size : " + (SetOperations.difference(m1, m2)));
        ...
        begin = System.currentTimeMillis();
        Mapping m3 = run(sourceMap, sourceMap, threshold);
        end = System.currentTimeMillis();
...
//        System.out.println("Mapping size : " + (SetOperations.difference(m1, m3)));
    ...
    private Map<String, Set<String>> generateRandomMap(int size) {
...
//        jm.test(10000, 10000, 0.7);
        jm.deduplicationTest(1000, 1.0);
