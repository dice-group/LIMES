...
public class NestedPlan extends Plan {
	...
	/**
	 * Used for parameterized t-norms
	 */
	...
	public double getCommandParameter() {
		return commandParameter;
	}

	public void setCommandParameter(double commandParameter) {
	...
	public NestedPlan clone() {
		...
		if (filteringInstruction != null) {
			Instruction cloneFilteringInstruction = filteringInstruction.clone();
	...
	public String finalPlan() {
		...
		if (isAtomic()) {
			return instructionList.get(0).getMeasureExpression() + "-"
			...
			if (operator == null) {
				...
				String op = "";
				if (operator.equals(Command.DIFF)) {
					op = "DIFFERENCE";
				} else if (operator.equals(Command.INTERSECTION)) {
					op = "INTERSECTION";
				} else if (operator.equals(Command.UNION)) {
					op = "UNION";
				} else if (operator.equals(Command.XOR)) {
					op = "XOR";
				}
	...
	public boolean equals(Object other) {
		...
		if (isAtomic() && o.isAtomic()) {
		...
		} else if (!isAtomic() && !o.isAtomic()) { // no instructionList
			if (operator == null && o.operator != null) {
				return false;
			}
			if (operator != null && o.operator == null) {
				return false;
			...
			if (operator == null && o.operator == null) {
				if (filteringInstruction == null && o.filteringInstruction == null) {
					// instructionList must be empty for complex plans
					// if it is not for any reason, the condition is still true
					if (instructionList.equals(o.instructionList)) {
						return subPlans.equals(o.subPlans);
					} else {
						return false;
					}
				}
				...
				if (filteringInstruction.equals(o.filteringInstruction)) {
					if (instructionList.equals(o.instructionList)) {
						return subPlans.equals(o.subPlans);
					} else {
						return false;
			...
			if (operator.equals(o.operator)) {
				...
				if (filteringInstruction == null && o.filteringInstruction == null) {
					if (instructionList.equals(o.instructionList)) {
						return subPlans.equals(o.subPlans)
								&& commandParameter == o.commandParameter;
					} else {
						return false;
					}
				}
	...
	public int hashCode() {
		...
		hb.append(isAtomic());
		hb.append(operator);
		hb.append(instructionList);
		hb.append(filteringInstruction);
		hb.append(subPlans);
		hb.append(commandParameter);
