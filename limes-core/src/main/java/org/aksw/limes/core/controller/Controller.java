...
public class Controller {
	...
	if (isAlgorithm) {
	    ...
	    LinkSpecification ls = new LinkSpecification(config.getMetricExpression(),
		...
		// 4.3. Planner
		ExecutionPlannerType executionPlanerType = ExecutionPlannerFactory
			.getExecutionPlannerType(config.getExecutionPlan().toLowerCase());
		Planner planner = ExecutionPlannerFactory.getPlanner(executionPlanerType, sourceCache, targetCache);
		assert planner != null;

		// 5. Execution
		ExecutionEngineType executionEngineType = ExecutionEngineFactory
			.getExecutionEngineType(config.getExecutionPlan().toLowerCase());
		...
		logger.info("Wrong Input Specification. Exiting..");
	    }
	}
