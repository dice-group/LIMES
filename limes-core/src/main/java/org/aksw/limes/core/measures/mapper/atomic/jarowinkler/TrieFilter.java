...
public class TrieFilter implements Runnable {
    ...
    public TrieFilter(Pair<List<String>, List<String>> lists, Map<String, Map<String, Double>> result, TrieFilterableStringMeasure metric, double threshold) {
    ...
    private void trieSearch (String b, List<String> partition) {
        ...
        Stack<MutableTriple<Stack<Character>, TrieNode, Integer>> searchStack =
                new Stack<>();
        // set inital algorithm stack
        for (Character key : trieRoot.children.keySet()) {
            //noinspection unchecked
            searchStack.add(new MutableTriple<>(
                    (Stack<Character>) referenceStack.clone(), trieRoot.children.get(key), matches));
        }
        // until the stack is empty, pop and evaluate
        while (!searchStack.isEmpty()) {
            MutableTriple<Stack<Character>, TrieNode, Integer> current =
                    searchStack.pop();
            int maxPossibleMatches = Math.min(current.getLeft().size(), maxLenInA - current.getMiddle().getLevel() + 1) + current.getRight();
            if (maxPossibleMatches >= metric.characterMatchLowerBound(bLen, minLenInA, threshold)) {
                ...
                if (currentOrder <= 0) {
                    if (currentOrder == 0) {
                        ...
                        if (current.getRight() >= metric.characterMatchLowerBound(bLen, current.getMiddle().getLevel(), threshold)
                                && current.getMiddle().data != null && current.getMiddle().data.size() > 0
                                && metric.characterFrequencyUpperBound(current.getMiddle().getLevel(), b.length(), current.getRight()) >= threshold) {
                            matchCandidateList.addAll(current.getMiddle().data);
                        }
                    }
                    for (Character key : current.getMiddle().children.keySet()) {
                        //noinspection unchecked
                        searchStack.push(new MutableTriple<>(
                                (Stack<Character>) current.getLeft().clone(), current.getMiddle().children.get(key), current.getRight().intValue()));
                    }
                } else {
                    current.getLeft().pop();
        ...
        if (matchCandidateList.size() > 0)
            matchCandidateListPairs.add(new MutablePair<List<String>, List<String>>(partition, new LinkedList<>(matchCandidateList)));
    }
