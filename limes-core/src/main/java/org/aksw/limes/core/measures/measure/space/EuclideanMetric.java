...
/**
 * Implements a spatial similarity based on the Euclidean distance. The similarity
 * is computed as 1/(1+d), where d is the distance between the two points. Consequently
...
//                        logger.warn("One of "+value1+" and "+value2+" is not a number.");
//                        logger.warn(a.getUri()+" or "+b.getUri()+" contains wrong data.");
...
//        logger.info("Similarity of "+a.getUri()+" and "+b.getUri()+" is "+1.0/Math.sqrt(sim));
        return 1.0/(1+Math.sqrt(sim));
    }
