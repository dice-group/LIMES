...
public class WombatComplete extends AWombat {
	...
	protected AMapping getNextExamples(int size) throws UnsupportedMLImplementationException {
		throw new UnsupportedMLImplementationException(getName());
	}
	...
	private Map<String, AMapping> computeClassifiersDiffPermutations(List<ExtendedClassifier> c) {
		...
		for (int i = 0; i < c.size(); i++) {
			for (int j = 0; j < c.size(); j++) {
				if (i != j) {
					AMapping m = CrispSetOperations.INSTANCE.difference(c.get(i).getMapping(), c.get(j).getMapping());
	...
	private Tree<RefinementNode> expandNode(Tree<RefinementNode> node) {
		...
		if (node.level() == 1) {
			...
			for (RefinementNode n : siblingNodes) {
				if (!inRefinementTree(n.getMetricExpression())) {
					node.getParent().addChild(new Tree<>(n));
				}
			}
		}
		if (isVerbose()) {
	...
	private List<RefinementNode> applyConOrDisjunction (Tree<RefinementNode> node, boolean useDisjunction) {
		...
		for (int i = 0; i < subMetricExpr.size(); i++) {
			...
			if (useDisjunction) {
				childMap = CrispSetOperations.INSTANCE.intersection(
						getMapingOfMetricExpression(childSubMetricExpr.get(0)),
						getMapingOfMetricExpression(childSubMetricExpr.get(1)));
			} else {
				childMap = CrispSetOperations.INSTANCE.union(getMapingOfMetricExpression(childSubMetricExpr.get(0)),
						getMapingOfMetricExpression(childSubMetricExpr.get(1)));
			}

			for (int k = 2; k < childSubMetricExpr.size(); k++) {
				childMetricExpr = operator + "(" + childMetricExpr + "," + childSubMetricExpr.get(k) + ")|0.0";
				childMap = CrispSetOperations.INSTANCE.intersection(childMap,
						getMapingOfMetricExpression(childSubMetricExpr.get(k)));
			}
	...
	private List<RefinementNode> createDisjunctionsWithDiffNodes(Tree<RefinementNode> node) {
		...
		for (String diffExpr : diffs.keySet()) {
			...
			AMapping childMap = CrispSetOperations.INSTANCE.union(nodeMaping, diffMapping);
			result.add(createNode(childMap, childMetricExpr));
		}
		return result;
	}
	...
	private List<RefinementNode> createConjunctionsWithDiffNodes(Tree<RefinementNode> node) {
		...
		for (String diffExpr : diffs.keySet()) {
			...
			AMapping childMap = CrispSetOperations.INSTANCE.intersection(nodeMaping, diffMapping);
			result.add(createNode(childMap, childMetricExpr));
		}
		return result;
	}
	...
	public double computeMaxRecall(List<ExtendedClassifier> classifiers) {
		...
		for (int i = 1; i < classifiers.size(); i++) {
			unionMaping = CrispSetOperations.INSTANCE.union(unionMaping, classifiers.get(i).getMapping());
		}
