 ...
 * trigram or levensthein. They all build a expression like
 * <i>similarityMeasure(resource1, resource2)|threshold</i>. Whereas the
 * threshold is ignored if the expression is atomic, i.e. the similarity measure
 * isn't part (argument) of another metric, such as <i>AND</i>). We now expect
 * atleast two parameters: 2 Terminals. Whereas the first is of sub return type
 * ResourceTerminalType.SOURCE, the second of ResourceTerminalType.TARGET. This
 * allows us to differentiate the different resources of the two endpoints and
 * avoid silly measures comparing the same resources of the same endpoint. If
 * mutation is turned on, the similarity command might be changed during the
 * evolution process to another one of the allowed measures.
 *
...
public class StringMeasure extends CommandGene implements IMutateable, ICloneable {
     ...
     */
    // TODO:add list of measures in constructor
    public StringMeasure(String opName, final GPConfiguration a_conf, Class<?> a_returnType, int a_subReturnType,
            boolean a_mutateable, Set<String> measures) throws InvalidConfigurationException {
        super(a_conf, 2, a_returnType, a_subReturnType, new int[] { ResourceTerminalType.STRINGPROPPAIR.intValue(),
                ResourceTerminalType.THRESHOLD.intValue(), });
        fillOperationSet(measures);
     ...
     * 
     * @param opName
     *            Name of the measure, e.g. "trigrams"
     */
     ...
     */
    private void fillOperationSet(Set<String> measures) {
        for(String atomicMeasure: measures){
            allowedOperations.add(atomicMeasure);
        }
     ...
     *
     * @return A random command out of the set of the allowed similarity
     *         measures
     * @throws InvalidConfigurationException
    ...
    public Object clone() {
        try {
            StringMeasure result = new StringMeasure(operationName, getGPConfiguration(), getReturnType(),
                    getSubReturnType(), m_mutateable, allowedOperations);
            return result;
        } catch (Throwable t) {
