...
public class LinkSpecSummary {
	...
	static List<NLGElement> nlgElements=new ArrayList<NLGElement>();
	static List<List<NLGElement>> allnlg =new ArrayList<List<NLGElement>>();
	static String leftProp;
	static String rightProp;
	/**
	 * @param linkSpec
	 * @param verbos
	...
	static List<NLGElement>fullMeasureNLG(LinkSpecification linkspec) throws UnsupportedMLImplementationException {
		...
		if(linkspec.isAtomic()) {
			...
			NPPhraseSpec condition=nlgFactory.createNounPhrase("condition");
			NPPhraseSpec one=nlgFactory.createNounPhrase("one");
			NPPhraseSpec both=nlgFactory.createNounPhrase("both");
			PPPhraseSpec pp = nlgFactory.createPrepositionPhrase();
			VPPhraseSpec v= nlgFactory.createVerbPhrase("hold");
			if(linkspec.getOperator().toString()=="OR") {
				one.addPreModifier("if");
				pp.addComplement(condition);
				pp.setPreposition("of");
				condition.setPlural(true);
				condition.setDeterminer("the");
				condition.addModifier("following");
				one.addComplement(pp);
				clause_2.setSubject(one);
				clause_2.setVerb(v);

			}

			if(linkspec.getOperator().toString()=="AND") {
				both.addPreModifier("if");
				pp.addComplement(condition);
				pp.setPreposition("of");
				condition.setPlural(true);
				condition.setDeterminer("the");
				condition.addModifier("following");
				both.addComplement(pp);
				clause_2.setSubject(both);
				clause_2.setVerb(v);	
			}
			if(linkspec.getOperator().toString()=="XOR") {
				both.addPreModifier("if");
				pp.addComplement(condition);
				pp.setPreposition("of");
				condition.setPlural(true);
			...
			if(linkspec.getOperator().toString()=="NOT") {
				...
				both.addComplement(pp);
				clause_2.setSubject(both);
				v.setFeature(Feature.NEGATED, true);
				clause_2.setVerb(v);	
			}
			clause_1.addComplement(linkspec.getOperator().toString().toLowerCase());
			coordinate_1.addComplement(clause_1);
			coordinate_1.addComplement(clause_2);
			clause_1=new SPhraseSpec(nlgFactory);
			clause_2=new SPhraseSpec(nlgFactory);
			...
			for (int i=0;i<linkspec.getChildren().size();i++) {
				...
				if(linkSpecification.isAtomic()) {
					...
					clause_3.addComplement(linkspec.getOperator().toString().toLowerCase());
					Realiser realiser_2 = new Realiser(lexicon);
					NLGElement realised_2 = realiser_2.realise(clause_3);
					String previous="";
					if (!previous.equals(realised_2.toString()) ) {               
						previous = realised_2.toString();
						nlgElements.add(realised_2);               
					}
					clause_3=new SPhraseSpec(nlgFactory);
					//System.out.println(" I am HERE "+linkSpecification.getParent().getOperator());
					NLGElement atomicMeasureNLG = atomicMeasureNLG(linkSpecification);
					String previousEle="";
					if (!previousEle.equals(atomicMeasureNLG.toString()) ) {               
						previousEle = atomicMeasureNLG.toString();
	...
	public static NLGElement atomicMeasureNLG(LinkSpecification linkSpec) throws UnsupportedMLImplementationException {
		...
		n1.setDeterminer("a");
		n1.addPostModifier(similarity);

		PhraseElement resource=nlgFactory.createNounPhrase("resource");
		resource.addPreModifier("same");
		resource.setFeature(Feature.POSSESSIVE, true);
		PhraseElement resourceValue=nlgFactory.createNounPhrase(leftProp);
		resourceValue.setFeature(InternalFeature.SPECIFIER, resource);
		//resourceValue.addPreModifier("same");
		...
		NPPhraseSpec theta=nlgFactory.createNounPhrase(linkSpec.getThreshold()*100 +"%");
		...
		DocumentElement sentence_1 = nlgFactory.createSentence();
		if(leftProp.contains(rightProp)) {
			clause_3.setSubject(sameSubj);
			clause_3.setVerb("have");
			//clause_3.addComplement("same");
			clause_3.setObject(resourceValue);

			if(linkSpec.getThreshold()==1.0) {
				//CoordinatedPhraseElement obj = nlgFactory.createCoordinatedPhrase(n1,"exact match");
				clause_1.setSubject("both");
				clause_1.setVerb("has");
				clause_1.setObject(n1);
				clause_2.setSubject("they");
				clause_2.setVerb("have");
				clause_2.setObject("exact match");
			}
			if(linkSpec.getThreshold()==0.0) {
				//CoordinatedPhraseElement obj = nlgFactory.createCoordinatedPhrase(n1,"complete mismatch");
				clause_1.setSubject("both");
				clause_1.setVerb("has");
				clause_1.setObject(n1);
				clause_2.setSubject("they");
				clause_2.setVerb("have");
				clause_2.setObject("complete mismatch");
			}
			if(linkSpec.getThreshold()>0.0&&linkSpec.getThreshold()<1.0) {
				//CoordinatedPhraseElement obj = nlgFactory.createCoordinatedPhrase(n1,pp);
				clause_1.setSubject("both");
				clause_1.setVerb("has");
				clause_1.setObject(n1);
				clause_2.setSubject("they");
				clause_2.setVerb("have");
				clause_2.addComplement("similarty");
				clause_2.setObject(theta);
			}
		}
		else {
			...
			if(linkSpec.getThreshold()==1.0) {
				//CoordinatedPhraseElement obj = nlgFactory.createCoordinatedPhrase(n1,"exact match");
				clause_1.setSubject(coordinate_1);
				clause_1.setVerb("has");
				clause_1.setObject(n1);
				clause_2.setSubject("they");
				clause_2.setVerb("have");
		...
		sentence_1.addComponent(coordinate_2);
		Realiser realiser_1 = new Realiser(lexicon);
