...
//	    //Test if there is a threshold <= 0  in which case this is an irrelevant subtree/leaf
	...
	if (left) {
	    ...
	    } else if (!(leftChild.startsWith("[" + classPositive) || leftChild.startsWith("[" + classNegative))) { 
		...
		return parseAtomicTree(tree.replace(rightChild, "[" + classPositive + "]").replace(leftChild, "[" + classNegative + "]"), true);
		//if it is a tree and not a leaf
	    } else if (!(rightChild.startsWith("[" + classPositive) || rightChild.startsWith("[" + classNegative))) { 
