...
class PartitionResult {
    ...
    /**
     *
     * @param l right bound of the left partition
     * @param r left bound of the right partition
     * @param f a flag indicating whether the token w is in the searching range
     * @param diff a flag indicating whether the probing token w is not found in
     * a record
     */
 ...
 * Siehe <a
 * href="http://www.cse.unsw.edu.au/~weiw/files/WWW08-PPJoin-Final.pdf">detaillierte
 * Beschreibung von PPJoin+</a>.
 *
...
public class PPJoinPlusPlus extends AtomicMapper {
     ...
     *
     * @param simFunct Ähnlichkeitsfunktion ('j' für Jaccard, 'c' für Cosine,
     * 't' für Trigram)
     * @param threshold Schwellwert aus dem Bereich [0,1]
     * @param maxDepth maximale Rekursionstiefe für die Suffix-Filterung
     * (optimal 2)
     * @param objects Array mit den zu untersuchenden Datensätzen
     * @return eine Liste der ähnlichen Objekte in der Form von ID-Paaren mit
     * dem berechneten Ähnlichkeitswert (ID entspricht der Stelle im Array
     * <i>objects</i>)
     */
            ...
            for (int j = 0; j < currentRec.tokens.length && j < currentRec.prefixLength; j++) {
                ...
                if (l != null) {
                    ...
                    while (iter.hasNext()) {
                        ...
                        if (tokensNumber2 >= sizeFilteringThreshold) {		//size filtering on |y|
                            ...
                            if (cf == null) {
                                if (ubound >= alpha) {
                                    //differs from paper because count starts at 0 not 1
                    ...
                    if (j < currentRec.midPrefix) {
                        l.add(new Position(currentRec, j));			//index the current prefix
                    }
                } else {
                    if (j < currentRec.midPrefix) {
...
//        logger.info("Mapping carried out using " + comparisons + " comparisons.");
    ...
    private int verification(Record currentRec, HashMap<Record, CandidateInfo> candidates) {
        ...
        for (Map.Entry e : candidates.entrySet()) {
            ...
            if (value.currentOverlap > 0) {
                ...
                } else {	//Fehler in Pseudocode; dieser Fall falsch behandelt --> Duplikate fehlen!
     ...
     *
     * @param x erster Datensatz
     * @param beginnX Position des Anfangstokens vom ersten Datensatz
     * @param y zweiter Datensatz
     * @param beginnY Position des Anfangstokens vom zweiten Datensatz
    ...
    private static int suffixFilter(Record x, int xBeginn, int xEnd, Record y, int yBeginn, int yEnd,
            int H_max, int depth) {
        ...
        int mid = yBeginn + (int) Math.ceil(1.0 * ySize / 2) - 1;		//index from 0 -> -1
        ...
        if (H > H_max) {
            ...
            int Hl = suffixFilter(x, xBeginn, pr.l, y, yBeginn, mid - 1,
                    H_max - Math.abs(xrSize - yrSize) - pr.diff, depth + 1);
     ...
     *
     * @param x a record
     * @param l the left bound of searching range
     * @param r the right bound of searching range
     * @param w a token
    ...
    private static Record[] tokenizer(String[] objects) {
        ...
        for (int i = 0; i < objects.length; i++) {
            ...
            //HasTable of tokens in the record <Sting> and their record frequency <Integer>
            ...
            for (int j = 0; j < tokensNumber; j++) {
                ...
                if (recordTokens.containsKey(token)) {
                    ...
                    if (allTokens.containsKey(token + "." + token_freq)) {		//character '.' is not allowed in tokens
