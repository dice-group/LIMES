...
public class Evaluator {
    ...
    /**
     * The qualitative measure evaluator that evaluates the predictions
     * generated by a machine learning algorithm using a gold standard and set
     * of measures
     */
    ...
    // --- --------
    // --- ACoreMLAlgorithm (concrete: EAGLE,WOMBAT,LION) u can retrieve it by
    // get() --------
    // --- --------
    // ---------------------------------------------------------------------------------------------------------
     ...
     *         corresponding to the algorithms, its implementation and used
     *         dataset
     */
    public List<EvaluationRun> evaluate(List<TaskAlgorithm> TaskAlgorithms, Set<TaskData> datasets, Set<EvaluatorType> QlMeasures,
	    Set<IQuantitativeMeasure> QnMeasures) {
	AMapping predictions = null;
	Map<EvaluatorType, Double> evaluationResults = null;
	try {
	    for (TaskAlgorithm tAlgorithm : TaskAlgorithms) { // iterate over
							      // algorithms
							      // tasks(type,algorithm,parameter)
		...
		for (TaskData dataset : datasets) { // iterate over
		    ...
		    // initialize the algorithm with source and target data,
		    // passing its parameters too ( if it is null in case of
		    // WOMBAT it will use its defaults)
		    tAlgorithm.getMlAlgorithm().init(null, dataset.source, dataset.target);

		    MLResults mlModel = null; // model resulting from the
					      // learning process
			...
			// sml.getNextExamples((int)Math.round(0.5*dataset.training.size()));
			for (int i = 0; i < 10; i++) {
		    ...
		    EvaluationRun er = new EvaluationRun(tAlgorithm.getMlAlgorithm().getName().replaceAll("\\s+", ""), tAlgorithm.getMlType().name()
			    .replaceAll("//s", ""), dataset.dataName.replaceAll("//s", ""), evaluationResults, mlModel.getLinkSpecification());
		    runsList.add(er);
		}
	    }
    ...
    public List<EvaluationRun> crossValidate(AMLAlgorithm algorithm, Set<TaskData> datasets, int folds, Set<EvaluatorType> qlMeasures,
	    Set<IQuantitativeMeasure> qnMeasures) {

	// Table<String, String, Map<EvaluatorType, Double>> evalTable =
	// HashBasedTable.create();// multimap stores
	// aglortihmName:datasetname:List of evaluations
	...
	for (TaskData dataset : datasets) {
		...
		// AN: Changed the type of mapping as the input is a type, not
		// the name
		// srcMap[i] = MappingFactory.createMapping(dataset.pairName +
		// "_mapping_" + i);
		// srcGold[i] = MappingFactory.createMapping(dataset.pairName +
		// "_goldstd_" + i);
		...
		for (Instance inst : srcParts[i].getAllInstances()) {
		    ...
		    // look for (s, t) belonging to mapping and create G1, ...,
		    // G10
		...
		for (Instance inst : srcParts[i].getAllInstances()) {
		    String uri = inst.getUri();
		    // look for (s, t) belonging to gold standard and create G1,
		    // ..., G10
		...
		EvaluationRun er = new EvaluationRun(algorithm.getName() + " - fold " + i, dataset.dataName, eval.evaluate(
			algorithm.predict(srcFolds[i], srcFolds[i], model), goldStandard, qlMeasures));
		runsList.add(er);
		// evalTable.put(algorithm.getName() + " - fold " + i,
		// dataset.dataName,
		// eval.evaluate(algorithm.predict(srcFolds[i], srcFolds[i],
		// model), goldStandard, qlMeasures));
	    }
	}
	return runsList;
	// return evalTable;
     ...
     * @return Mapping - The mappings from the predictions that exist in the
     *         gold standard
     */
     ...
     * results.rowKeySet()) { for (String dataset : results.columnKeySet()) {
    ...
    /*
     * It displays the results of evaluating a machine learning algorithm
     * regarding qualitative measures
     * 
