...
public class DuringMeasure extends TemporalMeasure {
	...
	if (new Double(split1[1]) < new Double(split2[1]) && new Double(split1[0]) > new Double(split2[0]))
	    sim = 1;
	else
	    sim = 0;
	...
	if (new Double(a.getProperty(endDate1).first()) < ((new Double(b.getProperty(endDate2).first())))
		&& new Double(a.getProperty(beginDate1).first()) > ((new Double(b.getProperty(beginDate2).first()))))
	    sim = 1;
	else
	    sim = 0;
