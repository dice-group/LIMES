...
public class EDJoin extends Mapper {
    ...
    public Mapping getMapping(Cache source, Cache target, String sourceVar,
        ...
        //make sure Q is set. Needed for indexing        
//        logger.info("Running EdJoin with thresold " + threshold);
        if (Q <= 1) {
            Q = 3;
        }
        ...
        //3.1 fill objects from source in entry. This is for indexing and sorting 
        // later on
            ...
            if (records[i].qGrams.length == 0 && records[i].s.length() > 0) {
                    ...
                    if ((sourceMap.containsKey(records[i].id) && targetMap.containsKey(records[j].id))
                        ...
                        if (Math.abs(x.length() - y.length()) <= threshold) {
                            ...
                            if (ed <= threshold) {
                                ...
                                //if (targetMap.containsKey(records[i].id) && sourceMap.containsKey(records[j].id)) 
                                {
                                    //get uris for ids
                                    {
                ...
                for (int j = 0; j < prefixLength; j++) {
                    ...
                    if (l != null) {
                        ...
                        l.add(new EdPosition(currentRec, loc));			//index the current prefix
                    } else {
...
//        logger.info("Computed " + allTokens.size() + " qgrams");
        for (int i = 0; i < records.length; i++) {
            Arrays.sort(records[i].qGrams);
        }
        Arrays.sort(records);
        s.Stoppen();
//        System.out.println("Indexing took " + s.Laufzeit() + " ms");
        return records;
    }

    private static int verification(Record currentRec, HashMap<Integer, Record> candidates,
            ArrayList<String> objects, int q, int threshold) {
        int count = 0;
        String id1, id2;
            ...
            if (xQGrams[i].token.id == yQGrams[j].token.id) {
                if (Math.abs(xQGrams[i].loc - yQGrams[j].loc) <= tau) {
                    ...
                    if (xQGrams[i].loc < yQGrams[j].loc) {
                        if (i == 0
                                || xQGrams[i].token.id != xQGrams[i - 1].token.id
                                || j == 0
                ...
                if (xQGrams[i].compareTo(yQGrams[j]) < 0) {		//<-- Fehler in Pseudocode
                    if (i == 0
                            || xQGrams[i].token.id != xQGrams[i - 1].token.id
                            || j == 0
                            || xQGrams[i].token.id != yQGrams[j - 1].token.id
    ...
    private static int contentFilter(String x, String y, MismatchingQGram[] Q, int tau, int q) {
        ...
        while (i < Q.length) {
            if (Q[i].loc - Q[i - 1].loc > 1) {
                epsilon = L1Distance(x, y, Q[j].loc, Q[i - 1].loc + q - 1)
                        + sumRightErrs(Q[i - 1].loc + q, iter);
                if (epsilon > 2 * tau) {
                    return 2 * tau + 1;
                }
                j = i;
        ...
        return L1Distance(x, y, Q[j].loc, Q[i - 1].loc + q - 1)
                + sumRightErrs(Q[i - 1].loc + q, iter);
    }
    ...
    public double getMappingSizeApproximation(int sourceSize, int targetSize, double theta, Language language) {
        if (language.equals(Language.DE)) {
            ...
            //return 11.64 + 0.0014 * sourceSize + 0.0014 * targetSize - 22.42 * theta;
        }
    }
