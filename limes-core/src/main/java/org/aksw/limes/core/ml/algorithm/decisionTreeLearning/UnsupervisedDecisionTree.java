...
//	private ACache leftTargetCache;
	...
	public UnsupervisedDecisionTree(DecisionTreeLearning dtl, ACache originalSourceCache, ACache originalTargetCache, PseudoFMeasure pseudoFMeasure, double minPropertyCoverage, double propertyLearningRate){
...
//        this.baseTargetCache = originalTargetCache;
	...
	public UnsupervisedDecisionTree(DecisionTreeLearning dtl, ACache originalSourceCache, ACache originalTargetCache, AMapping parentMapping, PseudoFMeasure pseudoFMeasure, double minPropertyCoverage, double propertyLearningRate,  UnsupervisedDecisionTree parent, boolean isLeftNode){
		if(parent == null){
...
//            	System.out.println("NULL");
		...
		if(maxDepth != this.depth){
			String measureExpression =  classifier.getMeasure() + "(x." + classifier.getSourceProperty() + ", y."  + classifier.getTargetProperty() + ")";
			usedMetrics.add(measureExpression);
//			createCachesFromMapping(classifier.getMapping());
			rightChild = new UnsupervisedDecisionTree(dtl, originalSourceCache, originalTargetCache, classifier.getMapping(), pseudoFMeasure, minPropertyCoverage, propertyLearningRate, this, false).buildTree(maxDepth);
//			AMapping lambdaMapping = executeAtomicMeasure(measureExpression, DecisionTreeLearning.threshold);
//			AMapping leftMapping = MappingOperations.difference(lambdaMapping, classifier.getMapping());
			leftChild = new UnsupervisedDecisionTree(dtl, originalSourceCache, originalTargetCache,classifier.getMapping(), pseudoFMeasure, minPropertyCoverage, propertyLearningRate, this, true).buildTree(maxDepth);
		}
		return this;
	}
...
//            }
	...
	private void calculatePathMapping(AMapping current){
		...
		if(nodeMappings.get(parent.getPathString()) != null){
			...
			// the parent node can no longer be a leave since it now has two childs
			if(isLeftNode){
	...
	public UnsupervisedDecisionTree prune(){
		...
		while(currentDepth != 0){
			...
			System.out.println(" PRUNE " + currentDepth);
			System.out.println(this.toString());
			System.out.println(totalFMeasure);
			predict();
		}
		return this;
	}
...
//			leftChild = tmpLeftChild;
		...
		System.out.println("totalFM: " + totalFMeasure);
		System.out.println("deleteLeft: " + deleteLeft);
		System.out.println("deleteRight: " + deleteRight);
		System.out.println("deleteBoth: " + deleteBoth);
		if(deleteBoth){
			this.rightChild = null;
			this.leftChild = null;
		}
...
//        	pathMappings.put(parent.getPathString(),parentMapping);
	...
	private double predict(){
		...
		PseudoRefFMeasure prfm = new PseudoRefFMeasure();
		return prfm.calculate(prediction, new GoldStandard(null, originalSourceCache.getAllUris(), originalTargetCache.getAllUris()),0.1);
	}
...
//			}
//		}
//		for (PairSimilar<String> propPair : dtl.getPropertyMapping().datePropPairs) {
...
//			}
//		}
//		for (PairSimilar<String> propPair : dtl.getPropertyMapping().pointsetPropPairs) {
...
//        logger.info("Done computing all classifiers.");
     ...
     */
    protected ExtendedClassifier findClassifier(String sourceProperty, String targetProperty, String measure) {
...
//        	logger.info("Skipping: " + measureExpression);
        ...
        for (double threshold = 1d; threshold > minPropertyCoverage; threshold = threshold * propertyLearningRate) {
            AMapping mapping = getMeasureMapping(measureExpression, threshold);
...
//            }
            ...
            if (maxFM < pfm ) { //only interested in largest threshold with highest F-Measure
                bestMapping = mapping;
                theta = threshold;
                maxFM = pfm;
            }
    ...
    private AMapping getMeasureMapping(String measureExpression, double threshold){
    	if(this.root){
    		AMapping mapping = executeAtomicMeasure(measureExpression, threshold);
            calculatedMappings.put(measureExpression + "|" + threshold, mapping);
            return mapping;
    	}
    	AMapping currentClassifierMapping = calculatedMappings.get(measureExpression + "|" + threshold);
    	if(this.isLeftNode){
    		AMapping differenceMapping = MappingOperations.difference(currentClassifierMapping, parentMapping);
//    		AMapping revdifferenceMapping = MappingOperations.difference(parentMapping, currentClassifierMapping);
//    		AMapping union = MappingOperations.union(currentClassifierMapping, parentMapping);
//    		AMapping intersection = MappingOperations.intersection(currentClassifierMapping, parentMapping);
//    		if(differenceMapping.size() > 0 || union.size() != parentMapping.size()){
//    		System.out.println("currentSize: " + currentClassifierMapping.size());
...
//    		System.out.println("currentClassifier: " + measureExpression + "|" + threshold);
//    		System.out.println("parentClassifier: " + parent.classifier.getMetricExpression());
//    		System.out.println("currentMapping = parentMapping: " + currentClassifierMapping.equals(parentMapping));
//    		}
...
//       	logger.info("Executing: " + measureExpression + "|" + threshold);
        ...
        ExecutionEngine ee = ExecutionEngineFactory.getEngine(ExecutionEngineType.DEFAULT, originalSourceCache, originalTargetCache, "?x", "?y");
        Plan plan = new Plan();
...
//       	logger.info("Executing: " + measureExpression + "|" + threshold);
    ...
    public String toString(){
    	String res = "";
    	if(classifier != null){
    		res += classifier.getMeasure() + TreeParser.delimiter + classifier.getSourceProperty() + "|" + classifier.getTargetProperty() + ": <= " + classifier.getThreshold() + ", > " + classifier.getThreshold() + "[";
    		if(leftChild != null){
...
//    	    AMapping b = execute("qgrams(x.http://www.okkam.org/ontology_person1.owl#soc_sec_id, y.http://www.okkam.org/ontology_person2.owl#soc_sec_id)", 0.48, c.getSourceCache(), c.getTargetCache());
//    	    AMapping a = execute("qgrams(x.http://www.okkam.org/ontology_person1.owl#date_of_birth, y.http://www.okkam.org/ontology_person2.owl#date_of_birth)", 1.0, c.getSourceCache(), c.getTargetCache());
...
//			MLResults resWom = wombat.asUnsupervised().learn(new PseudoFMeasure());
			long end = System.currentTimeMillis();
//			System.out.println(resWom.getLinkSpecification());
//			System.out.println("FMeasure: " + new FMeasure().calculate(wombat.predict(c.getSourceCache(), c.getTargetCache(), resWom), new GoldStandard(c.getReferenceMapping(),c.getSourceCache(), c.getTargetCache())));
...
//			System.out.println(res.getLinkSpecification());
			System.out.println("FMeasure: " + new FMeasure().calculate(dtl.predict(c.getSourceCache(), c.getTargetCache(), res), new GoldStandard(c.getReferenceMapping(),c.getSourceCache(), c.getTargetCache())));
			System.out.println("Time: " + (end - start));
...
//    	for(String data: datasets){
//    		EvaluationData c = DataSetChooser.getData(data);
//    		ExecutionEngine ee = ExecutionEngineFactory.getEngine(ExecutionEngineType.DEFAULT, c.getSourceCache(), c.getTargetCache(), "?x", "?y");
//    		Configuration config = c.getConfigReader().read();
//    		LinkSpecification ls = new LinkSpecification(config.getMetricExpression(), config.getAcceptanceThreshold());
//    		if(data.equals("drugs")){
//    			ls = new LinkSpecification("jaccard(x.name,y.name)", 1.0);
//    		}
//    		AMapping predictions = ee.execute(ls, new DynamicPlanner(c.getSourceCache(), c.getTargetCache()));
...
//    		double f = fm.calculate(predictions, new GoldStandard(c.getReferenceMapping()));
//    		System.out.println("F: " + f); 
//    		double pf = pfm.calculate(predictions, new GoldStandard(null, c.getSourceCache().getAllUris(), c.getTargetCache().getAllUris()));
//    		System.out.println("PF: " + pf); 
//    		PseudoRefFMeasure prfm = new PseudoRefFMeasure();
//    		double customf = prfm.calculate(predictions, new GoldStandard(null, c.getSourceCache().getAllUris(), c.getTargetCache().getAllUris()));
