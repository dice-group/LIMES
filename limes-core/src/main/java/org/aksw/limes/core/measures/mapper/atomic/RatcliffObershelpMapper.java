...
import de.uni_leipzig.simba.measures.string.RatcliffObershelpMeasure;
...
public class RatcliffObershelpMapper implements AtomicMapper {
    ...
    public Mapping getMapping(Cache source, Cache target, String sourceVar, String targetVar, String expression, double threshold) {
        ...
        similarityBook = new
                ConcurrentHashMap<>(listA.size(), 1.0f);
        ...
        if (metric.lengthUpperBound(1, threshold) != -1) {
            List<ImmutableTriple<Integer, Integer, Integer>> sliceBoundaries =
                    metric.getPartitionBounds(blue.get(blue.size() - 1).length(), threshold);
        ...
        logger.info("Initializing Threadpool for " + String.valueOf(Runtime.getRuntime().availableProcessors()) + " threads.");
