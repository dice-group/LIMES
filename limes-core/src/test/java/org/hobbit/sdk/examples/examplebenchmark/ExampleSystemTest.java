...
public class ExampleSystemTest extends EnvironmentVariablesWrapper {
	...
	public void init(boolean useCachedImages) throws Exception {
		...
		benchmarkBuilder = new BenchmarkDockerBuilder(
			new PullBasedDockersBuilder(BENCHMARK_IMAGE_NAME));
		dataGeneratorBuilder = new DataGenDockerBuilder(
			new PullBasedDockersBuilder(DATAGEN_IMAGE_NAME));
		taskGeneratorBuilder = new TaskGenDockerBuilder(
			new PullBasedDockersBuilder(TASKGEN_IMAGE_NAME));
		evalStorageBuilder = new EvalStorageDockerBuilder(
			new PullBasedDockersBuilder(EVAL_STORAGE_IMAGE_NAME));
		evalModuleBuilder = new EvalModuleDockerBuilder(
			new PullBasedDockersBuilder(EVALMODULE_IMAGE_NAME));
