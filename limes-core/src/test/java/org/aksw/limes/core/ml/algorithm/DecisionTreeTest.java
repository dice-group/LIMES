...
public class DecisionTreeTest {
	...
	public void prepareData() {
		...
		try {
			dtl = MLAlgorithmFactory.createMLAlgorithm(Dragon.class, MLImplementationType.SUPERVISED_BATCH);
			dtl2 = MLAlgorithmFactory.createMLAlgorithm(Dragon.class, MLImplementationType.SUPERVISED_BATCH);
	...
	IllegalArgumentException, IllegalAccessException {
		...
		assertEquals("Source caches do not match", sourceCache,
				sourceCacheField.get(((Dragon) dtl.getMl()).root));
		System.out.println(res.getLinkSpecification());
		System.out.println(
				"FMeasure: " + new FMeasure().calculate(dtl.predict(sourceCache, targetCache, res),
						new GoldStandard(c.getReferenceMapping())));
	}

	public static AMapping getTrainingData(AMapping full) {
		int sliceSizeWanted = full.size() - (int) Math.ceil(full.getSize() / 10.0);
		...
		System.out.println("got: " + CrispSetOperations.INSTANCE.intersection(slice, full).size() + " wanted: "
				+ sliceSizeWanted
				+ " full: " + full.size());
		return slice;
	}
	...
	public void testClone1()
		...
		assertEquals("Tree is incorrect", goldTree2.toString(), test.toString());
	}
	...
	IllegalArgumentException, InvocationTargetException, NoSuchFieldException, InstantiationException {
		...
		assertEquals("Tree is incorrect", goldTree2.toString(), root.toString());
	}
	...
	IllegalArgumentException, InvocationTargetException, NoSuchFieldException, InstantiationException {
		...
		assertEquals("Tree is incorrect", goldTree2.toString(), root2.toString());
	}
	...
	IllegalArgumentException, InvocationTargetException, NoSuchFieldException, InstantiationException {
		Method getRootNode = DecisionTree.class.getDeclaredMethod("getRootNode", new Class[0]);
		getRootNode.setAccessible(true);
		DecisionTree detachedNode = createNode((Dragon) dtl2.getMl(), "jaccard",
	...
	IllegalArgumentException, InvocationTargetException {
		Method calculatePathMappings = DecisionTree.class.getDeclaredMethod("calculatePathMappings",
				new Class[] { DecisionTree.class });
		calculatePathMappings.setAccessible(true);
		List<String> pathstrings = (List<String>) calculatePathMappings.invoke(goldTreeMapping,
				goldTreeMapping);
	...
	private DecisionTree createRoot(Dragon decisionTreeLearning, String measure, String sourceProperty,
			String targetProperty, double threshold)
					throws NoSuchFieldException, SecurityException, IllegalArgumentException, IllegalAccessException {
		DecisionTree tree = new DecisionTree(decisionTreeLearning, sourceCache, targetCache, pfm,
				...
				MappingFactory.createDefaultMapping(), c.getPropertyMapping());
	...
	private DecisionTree createNode(Dragon decisionTreeLearning, String measure, String sourceProperty,
		...
		Class<?>[] parameterTypes = { Dragon.class, ACache.class, ACache.class, PseudoFMeasure.class,
		...
		DecisionTree tree = constructor.newInstance(decisionTreeLearning, sourceCache, targetCache,
				pfm, (double) decisionTreeLearning.getParameter(Dragon.PARAMETER_MIN_PROPERTY_COVERAGE),
